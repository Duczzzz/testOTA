...
void loop() {
  ...
  */
  // Đọc giá trị ngưỡng cảnh báo kênh 1
  float threshold = 0.0;
  if (Firebase.getFloat(fbdo, "/users/luong/Card/Data-6-CB1")) {
    threshold = fbdo.floatData();
  }

  // Giả sử giá trị kênh 1 được lấy từ cảm biến BME280 (ví dụ: nhiệt độ)
  float channelValue = bme.readTemperature();

  // Ghi giá trị kênh 1 lên Firebase
  Firebase.setFloat(fbdo, "/users/luong/Card/Data-6-1", channelValue);

  // Kiểm tra cảnh báo và cập nhật đèn cảnh báo
  int warnLedState = (channelValue > threshold) ? 1 : 0;
  Firebase.setInt(fbdo, "/users/luong/Card/Data-6-Warnled", warnLedState);

  // Cập nhật LED nội bộ để phản ánh trạng thái cảnh báo
  if (warnLedState == 1) {
    led.setPixelColor(0, led.Color(255, 0, 0)); // Đỏ khi cảnh báo
  } else {
    led.setPixelColor(0, led.Color(0, 255, 0)); // Xanh khi bình thường
  }
  led.show();
  ...
  display.clearDisplay();
  display.setCursor(0,0);
  display.printf("Chan1: %.2f", channelValue);
  display.setCursor(0,15);
  display.printf("Thresh: %.2f", threshold);
  display.setCursor(0,30);
  display.print("Warn LED: ");
  display.print(warnLedState);
  display.display();
